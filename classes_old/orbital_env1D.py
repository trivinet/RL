...
class OrbitalEnv1D(gym.Env):
    ...
    def step(self, action):
        ...
        if abs(self.state[0] - self.goal[0]) > 3000:
          done = True
        ...
        if done and self.steps_in_goal >= 100:
          ...
          print(f"✅ Success #{self.success_counter}")
    ...
    def _compute_reward(self, state):
      ...
      a_error = min(abs(goal[0] - a), 3000)  # cap to avoid extreme reward magnitude
      reward = -a_error / 10

      prev_a = self.trajectory[-2][0] if len(self.trajectory) > 1 else a
      da = a - prev_a
      expected_direction = np.sign(goal[0] - a)

      if np.sign(da) == expected_direction:
          reward += 3.0  # ✔️ reward just for moving a in the right direction
      else:
          reward -= 3.0  # ❌ penalize when a is drifting away
      
      if expected_direction != np.sign(da) and abs(goal[0] - a) > 200:
        reward -= 50  # large punishment for drifting further away

      fs = self.last_throttle  # really the signed Fs action

      thrust_direction = np.sign(fs)
      if thrust_direction == np.sign(goal[0] - a):
          reward += 1.0 * abs(fs)
      else:
          reward -= 2.0 * abs(fs)

      if a_error < 20 and abs(fs) < 0.1:
          reward += 0.5
